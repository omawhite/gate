...
class OidRolesExtractorSpec extends Specification {
  ...
  @Unroll
  X509RolesExtractor "should return ldap roles - #description"() {
    given:
    String oid = "1.2.840.10070.8.1"
    def extractor = new OidRolesExtractor(oid: oid)
    ...
    "no roles"    | "no-groups.crt"    | 0
    "one role"    | "one-group.crt"    | 1
    "more roles"  | "two-groups.crt"   | 2
  }
}
